...
export function documentLink(href, base) {
  if (/^[A-Za-z-]+\.md(?:#.*)?$/.test(href)) {
    ...
    return (
      base +
      "/" +
      (name === "Home.md" ? "" : name.slice(0, -3) + "/") +
      (hash ? "#" + hash : "")
    );
  }
  if (/^(diagrams|reference)\/[A-Za-z0-9_.-]+$/.test(href))
    return base + "/" + href;
  return href;
}
export function renderMarkdown(source, base) {
  ...
  md.renderer.rules.link_open = (tokens, i, options, env, self) => {
    tokens[i].attrSet(
      "href",
      documentLink(tokens[i].attrGet("href") || "", base),
    );
  ...
  md.renderer.rules.image = (tokens, i, options, env, self) => {
    ...
    if (!/^diagrams\/[a-z-]+\.svg$/.test(src))
      throw Error("Unexpected wiki image source");
    const href = documentLink(src, base);
    tokens[i].attrSet("src", href);
    tokens[i].attrSet("loading", "lazy");
    return (
      '<a class="diagram" href="' +
      escapeHTML(href) +
      '">' +
      image(tokens, i, options, env, self) +
      "<span>Open full-size diagram</span></a>"
    );
  };
  md.renderer.rules.table_open = () =>
    '<div class="table-scroll" tabindex="0" role="region" aria-label="Scrollable documentation table"><table>';
  md.renderer.rules.table_close = () => "</table></div>";
  md.renderer.rules.heading_open = (tokens, i, options, env, self) => {
    const slug = tokens[i + 1].content
      .toLowerCase()
      .replace(/[^a-z0-9]+/g, "-")
      .replace(/^-|-$/g, "");
...
export function build() {
  const config = JSON.parse(fs.readFileSync("site.config.json", "utf8")),
    current = JSON.parse(fs.readFileSync("generated/current.json", "utf8"));
  if (!/^[a-f0-9]{40}$/.test(current.sourceRevision))
    throw Error("Invalid application revision");
  const base = config.basePath;
  fs.rmSync("dist", { recursive: true, force: true });
  fs.mkdirSync("dist", { recursive: true });
  fs.cpSync("assets", "dist/assets", { recursive: true });
  fs.cpSync("diagrams", "dist/diagrams", { recursive: true });
  fs.cpSync("reference", "dist/reference", { recursive: true });
  for (const [id, title] of config.chapters) {
    const source = fs.readFileSync("pages/" + id + ".md", "utf8");
    const nav = config.chapters
      .map(
        ([key, label]) =>
          `<a href="${base}/${key === "Home" ? "" : key + "/"}" ${key === id ? 'aria-current="page"' : ""}>${escapeHTML(label)}</a>`,
      )
      .join("\n");
  ...
  fs.writeFileSync(
    "dist/404.html",
    `<!doctype html><html lang="en"><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1"><title>Page not found · KubeQuest wiki</title><link rel="stylesheet" href="${base}/assets/style.css"><main class="not-found"><h1>This wiki page has moved.</h1><p><a href="${base}/">Open the wiki index</a> or <a href="${config.portal}">return to KubeQuest</a>.</p></main></html>`,
  );
  console.log(
    `Built ${config.chapters.length} static chapters and 16 UML diagrams for ${current.sourceRevision}.`,
  );
}
if (
  process.argv[1] &&
